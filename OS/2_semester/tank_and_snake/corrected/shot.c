...
//display each part of the tank separately
	...
	mvaddch(p->x-1, p->y-1, p->new_tank[0][0]);
	mvaddch(p->x-1, p->y, p->new_tank[0][1]);
	mvaddch(p->x-1, p->y+1, p->new_tank[0][2]);
	mvaddch(p->x, p->y-1, p->new_tank[1][0]);
	mvaddch(p->x, p->y, p->new_tank[1][1]);
	mvaddch(p->x, p->y+1, p->new_tank[1][2]);
	mvaddch(p->x+1, p->y-1, p->new_tank[2][0]);
	mvaddch(p->x+1, p->y, p->new_tank[2][1]);
	mvaddch(p->x+1, p->y+1, p->new_tank[2][2]);

	mvaddch(p->x_tower, p->y_tower, p->tower);

	if(p->num_bul == 1)
	{
		if(p->x_bul >= 0 && p->x_bul <= LINES-1 && p->y_bul >=0 && p->y_bul <= COLS-1)
		{
			mvaddch(p->x_bul, p->y_bul, 'o');
			switch(p->bul_tower){
			case 1:
				p->x_bul--;
				break;
...
//display the tank on the screen, then display the turret
	...
	print_tank(&tank_p);
	mvaddch(tank_p.x_tower, tank_p.y_tower, tank_p.tower);
	refresh();
	curs_set(0);
